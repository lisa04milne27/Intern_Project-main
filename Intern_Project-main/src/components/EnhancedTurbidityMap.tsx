...
export const EnhancedTurbidityMap: React.FC<EnhancedTurbidityMapProps> = ({
  ...
  // Use TTN data for the first sensor if available
  const sensorsWithTTN = sensors.map((sensor, idx) => {
    if (idx === 0 && ttnData?.uplink_message?.decoded_payload) {
      return {
        ...sensor,
        turbidity: ttnData.uplink_message.decoded_payload.turbidity ?? sensor.turbidity,
        waterTemperature: ttnData.uplink_message.decoded_payload.temperature ?? sensor.waterTemperature,
        location: ttnData.location ?? sensor.location,
      };
    }
    return sensor;
  });
